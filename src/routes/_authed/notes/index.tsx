...
function RouteComponent() {
	...
	const { data: history, refetch: refetchHistory } = useQuery({
		...
		queryFn: async () => {
			...
			const { data, error } = await getTreaty().notes({ id: note.id }).history.get();
			if (error) throw error;
			return data;
		},
		...
		const timer = setTimeout(async () => {
			const noteContent =
				note && typeof note === "object" && "content" in note
					...
					: "";
			const noteMood =
				note && typeof note === "object" && "mood" in note
					? (note as any).mood
					: null;
	...
	const handleRestore = async (historyId: string) => {
		...
		try {
			const response = await fetch(
				`/api/notes/${note.id}/restore/${historyId}`,
				{
					method: "POST",
					credentials: "include",
				},
								...
								{recentNotes &&
									...
									<div className="col-span-full text-center py-12">
										...
										<p className="text-gray-500 dark:text-gray-400">
											No notes yet. Switch to Note view to create your first
											note!
										</p>
									</div>
								)}
