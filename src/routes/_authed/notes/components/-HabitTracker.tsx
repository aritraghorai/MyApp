...
export function HabitTracker({ date }: HabitTrackerProps) {
    ...
    const [logMetadata, setLogMetadata] = useState<Record<string, any> | null>(null);
    ...
    const toggleHabit = useMutation({
        mutationFn: async ({ habitId, completed, metadata, notes }: { habitId: string; completed: boolean; metadata?: Record<string, any>; notes?: string }) => {
    ...
    const handleHabitClick = (habit: Habit) => {
        ...
        if (!isCompleted && habit.metadataSchema && Object.keys(habit.metadataSchema).length > 0) {
    ...
    return (
        <>
            ...
            <Dialog open={!!loggingHabit} onOpenChange={(open) => !open && setLoggingHabit(null)}>
