...
export function TransactionForm({
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (formData.type === "CC_PAYMENT") {
			...
			if (initialData) {
				...
				// @ts-expect-error
				const { error } =
					await getTreaty().transactions.payment.post(paymentPayload);
				if (error) {
					console.error(error);
	...
	return (
		<form
			...
			className="space-y-6 max-w-2xl mx-auto p-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700"
		>
						...
						<div className="space-y-2">
							...
							<div className="flex gap-2">
								<Select
									value={
										formData.billingCycle
											? formData.billingCycle.split("_")[0]
											: ""
									}
									onValueChange={(month) => {
										const year = formData.billingCycle
											? formData.billingCycle.split("_")[1]
											: new Date().getFullYear().toString();
										setFormData({
											...formData,
											billingCycle: `${month}_${year}`,
										});
									}}
								>
									<SelectTrigger className="w-full bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white">
								...
								<Select
									...
									onValueChange={(year) => {
										const month = formData.billingCycle
											? formData.billingCycle.split("_")[0]
