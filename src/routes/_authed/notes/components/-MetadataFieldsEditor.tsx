...
export function MetadataFieldsEditor({
	...
	const applyTemplate = (templateKey: string) => {
		const template =
			HABIT_TEMPLATES[templateKey as keyof typeof HABIT_TEMPLATES];
		if (!template) return;

		onChange(template.metadataSchema);

		// Parse template schema into fields
		const templateFields = Object.entries(template.metadataSchema).map(
			([name, typeStr]) => {
				const [type, labelPart] = typeStr.split(":");
				let label = name;
				let unit: string | undefined;
		...
		<div className="space-y-4">
			...
			{fields.length === 0 && (
				<p className="text-xs text-gray-500 dark:text-gray-400 text-center">
					No metadata fields defined. Use a template or add custom fields to
					track metrics.
				</p>
			)}
		</div>
