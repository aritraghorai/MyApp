...
export function TemplatePicker({ onApplyTemplate, currentDate }: TemplatePickerProps) {
    const [open, setOpen] = useState(false);
    const [manageOpen, setManageOpen] = useState(false);
    const [editingTemplate, setEditingTemplate] = useState<CustomTemplate | null>(null);
    const [formData, setFormData] = useState({ name: "", category: "Personal", content: "" });
    ...
    const saveTemplate = useMutation({
        mutationFn: async (template: { id?: string; name: string; category: string; content: string }) => {
            const url = template.id ? `/api/notes/templates/${template.id}` : "/api/notes/templates";
    ...
    const handleSelectTemplate = (templateKey: string, isCustom = false) => {
        ...
        console.log("Template selected, rendered content:", renderedContent.substring(0, 100));
    ...
    const personalTemplates = templates.filter(([_, t]) => t.category === "Personal");
    const workTemplates = templates.filter(([_, t]) => t.category === "Work");
    const customPersonal = customTemplates.filter((t) => t.category === "Personal");
    ...
    return (
        <>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
                <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Empty State */}
                        {(personalTemplates.length === 0 && customPersonal.length === 0 && workTemplates.length === 0 && customWork.length === 0) && (
                            <div className="text-center py-4 text-gray-500 dark:text-gray-400 text-sm">
                                <FileText size={32} className="mx-auto mb-2 opacity-30" />
                                <p>No templates available. Create one in the manage section!</p>
                            </div>
                        )}
                    </div>
                </DialogContent>
            ...
            <Dialog open={manageOpen} onOpenChange={setManageOpen}>
                <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingTemplate ? "Edit Template" : "Manage Custom Templates"}</DialogTitle>
                        <DialogDescription>
                            {editingTemplate ? "Modify your custom template." : "Create, edit, or delete your custom templates."}
                        </DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4">
                        {!editingTemplate && (
                            <form onSubmit={(e) => { e.preventDefault(); handleSaveTemplate(); }} className="space-y-4 p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
                                ...
                                {editingTemplate && (
                                    <Button type="button" variant="outline" onClick={() => { setEditingTemplate(null); setFormData({ name: "", category: "Personal", content: "" }); }} className="ml-2">
                                        Cancel Edit
                                    </Button>
                                )}
                            </form>
                        ...
                        <h4 className="font-semibold text-lg mt-6">Your Custom Templates</h4>
                        {customTemplates.length === 0 ? (
                            <p className="text-gray-500 dark:text-gray-400">No custom templates yet. Create one above!</p>
                        ) : (
                            <div className="space-y-2">
                                {customTemplates.map((template) => (
                                    <div key={template.id} className="flex items-center justify-between p-3 border rounded-md bg-white dark:bg-gray-900">
                                        <div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Button variant="destructive" size="sm" onClick={() => deleteTemplate.mutate(template.id)} disabled={deleteTemplate.isPending}>
                                                <Trash2 size={16} />
                                            </Button>
                                        </div>
                                    </div>
