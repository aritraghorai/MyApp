...
export function TodoDashboard({ noteId, noteContent, noteDate, onTodoChange }: TodoDashboardProps) {
    ...
    const { data: todos = [], isLoading } = useQuery({
        ...
        queryFn: async () => {
            const response = await fetch(`/api/notes/todos/${noteId}`, {
                credentials: "include",
            });
    ...
    const toggleTodo = useMutation({
        mutationFn: async ({ id, content, completed }: { id: string; content: string; completed: boolean }) => {
            // First update the todo in database
            const response = await fetch(`/api/notes/todos/${id}`, {
                method: "PATCH",
            ...
            // Then sync the change back to the note's markdown
            const updatedMarkdown = updateTodoInMarkdown(noteContent, content, completed);
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3">
                <div className="bg-gradient-to-br from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-lg p-3 border border-purple-200 dark:border-purple-800">
                ...
                {filteredTodos.map((todo) => (
                    ...
                    >
                        ...
                        <button
                            type="button"
                            onClick={() =>
                                toggleTodo.mutate({ id: todo.id, content: todo.content, completed: !todo.completed })
                            }
