...
export function AllTodos() {
    ...
    const { data: todos = [], isLoading } = useQuery({
        queryKey: ["all-todos"],
        queryFn: async () => {
            const response = await fetch("/api/notes/todos/incomplete", {
                credentials: "include",
            });
    ...
    const toggleTodo = useMutation({
        mutationFn: async ({ id, completed, content, noteId, noteDate }: {
            id: string;
            completed: boolean;
            content: string;
            noteId: string;
            ...
            const todoResponse = await fetch(`/api/notes/todos/${id}`, {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                credentials: "include",
                body: JSON.stringify({ completed }),
            });
            if (!todoResponse.ok) throw new Error("Failed to update todo");

            // Then, fetch the note and update its markdown
            const noteResponse = await fetch(`/api/notes?date=${noteDate}`, {
                credentials: "include",
            });
            ...
            if (noteData && noteData.content) {
                // Update the markdown to reflect the todo change
                const lines = noteData.content.split('\n');
                const updatedLines = lines.map((line: string) => {
                    // Match the todo content (removing priority tags for comparison)
                    const cleanLine = line.replace(/#(high|medium|low)/gi, '').trim();
                    const cleanContent = content.replace(/#(high|medium|low)/gi, '').trim();

                    if (cleanLine.includes(cleanContent)) {
                        if (completed) {
    ...
    const stats = {
        ...
        overdue: todos.filter(
            (t) => !t.completed && isPast(startOfDay(new Date(t.note.date))) && format(new Date(t.note.date), "yyyy-MM-dd") !== format(new Date(), "yyyy-MM-dd")
        ).length,
    };
                                        ...
                                        >
                                            <button
                                                ...
                                                onClick={() => toggleTodo.mutate({
                                                    ...
                                                    noteId: todo.noteId,
                                                    noteDate: format(new Date(todo.note.date), "yyyy-MM-dd")
                                                })}
